#NOTE: modified inplace, use copy() to protect it.
extend = (base) -> (defaults...) ->
  for d in defaults when d?
  #NOTE: null values will be replaced if a default value exists.
...
watched_browserify = do ->
  ...
  (opts, piper) ->
    ...
    w = watchify browserify extend({entries: src, debug: true}) opts,
      watchify.args
